...
export const ImportExcelModal = ({ sizeButton }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    const { error, message, errors, total_errors, remaining_errors } =
      await importExcel({ file: selectedFile });
    setIsLoading(false);
                        ...
                        description={
                          <div className="space-y-1">
                              ...
                              .map((error, index) => (
                                <div key={index} className="text-sm">
                                  {String(error.error)
                                    ...
                                    .map((line, i) => (
                                      <div key={i}>
                                        Fila {error.fila}: {line}
                                      </div>
                                    ))}
                                </div>
                              ))}
                            {showAll && (
                              <div className="text-sm">{`${remainingErrors} errores más.`}</div>
                            )}
                            {validationErrors.length > 4 && !showAll && (
                              ...
                              >
                                Mostrar {validationErrors.length - 4} errores
                                más...
                              </button>
                            )}
                            {showAll && (
