...
import { deleteUser, User } from "@/modules/admin/users";

interface Props {
  user: User;
}

export const DeleteModal = ({ user }: Props) => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [isLoading, setIsLoading] = useState(false);

  const handleDelete = async () => {
    setIsLoading(true);
    const { error, message } = await deleteUser(user.id.toString());
    setIsLoading(false);
    ...
    if (error) {
      ...
      addToast({
        ...
        description: message || "Error al eliminar usuario",
        color: "danger",
        timeout: 2000,
        variant: "bordered",
  ...
  return (
    <>
      <Tooltip color="warning" content="Eliminar usuario">
        <Button
          onPress={onOpen}
          radius="full"
          variant="light"
