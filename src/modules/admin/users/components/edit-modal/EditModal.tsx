...
import { addToast, Button, Form, Input, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Radio, RadioGroup, Select, Selection, SelectItem, Tooltip, useDisclosure } from "@heroui/react";
import { Add01Icon, Edit02Icon } from "hugeicons-react";
import { useEffect, useRef, useState } from "react";
import { addUser, editUser, User, UsersConfigResponse } from "@/modules/admin/users";
...
export const EditModal = ({ textButton, sizeButton, usersConfig, user }: Props) => {
    ...
    const [documentType, setDocumentType] = useState<string>(user.type_document || '');
    const [documentNumber, setDocumentNumber] = useState<string>(user.n_document || '');
                            ...
                            >
                                ...
                                <ModalBody>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 gap-y-4">
                                        <Input
                                            name="name"
                                            radius="lg" label="Nombre" placeholder="Ingrese el nombre del usuario" type="text" variant="bordered"
                                        ...
                                        <Input
                                            name="surname"
                                            radius="lg" label="Apellido" placeholder="Ingrese el apellido del usuario" type="text" variant="bordered"
                                            value={surname} onValueChange={setSurname}
                                            onFocusChange={() => {
                                                if (name) {
                                                    setName(name.charAt(0).toUpperCase() + name.slice(1).toLowerCase());
                                                }
                                        ...
                                        <Input
                                            name="email"
                                            radius="lg" label="Email" placeholder="Ingrese el correo del usuario" type="email" variant="bordered"
                                            value={email} onValueChange={setEmail}
                                        />
                                        <Input radius="lg" label="Teléfono" placeholder="Ingrese el teléfono del usuario" type="text" variant="bordered" value={phone} onValueChange={setPhone} />
                                        <Select
                                            radius="lg"
                                        ...
                                        >
                                            ...
                                            <SelectItem key="PASAPORTE">
                                                Pasaporte
                                            </SelectItem>
                                        </Select>
                                        <Input
                                            radius="lg" label="Número de Documento" placeholder="Ingrese el nombre del rol" type="text" variant="bordered"
                                            value={documentNumber} onValueChange={setDocumentNumber}
                                        ...
                                        <Input
                                            name="password"
                                            radius="lg" label="Contraseña" type="password" variant="bordered"
                                            value={password} onValueChange={setPassword}
                                        />
                                        <Input
                                            name="password_confirmation"
                                            radius="lg" label="Confirmar contraseña" type="password" variant="bordered"
                                            value={passwordConfirmation} onValueChange={setPasswordConfirmation}
                                            isInvalid={password && password !== passwordConfirmation ? true : false}
