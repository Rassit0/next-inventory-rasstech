...
import { Warehouse, deleteWarehouse } from "@/modules/admin/warehouses";

interface Props {
  warehouse: Warehouse;
}

export const DeleteModal = ({ warehouse }: Props) => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [isLoading, setIsLoading] = useState(false);

  const handleDelete = async () => {
    setIsLoading(true);
    const { error, message } = await deleteWarehouse(warehouse.id.toString());
    setIsLoading(false);
    ...
    if (error) {
      ...
      addToast({
        ...
        description: message || "Error al eliminar el almacén",
        color: "danger",
        timeout: 2000,
        variant: "bordered",
