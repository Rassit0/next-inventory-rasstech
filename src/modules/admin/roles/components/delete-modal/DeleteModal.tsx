...
import { deleteRole, Role } from "@/modules/admin/roles";

interface Props {
  role: Role;
}

export const DeleteModal = ({ role }: Props) => {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [isLoading, setIsLoading] = useState(false);

  const handleDelete = async () => {
    setIsLoading(true);
    const { error, message } = await deleteRole(role.id.toString());
    setIsLoading(false);
    ...
    if (error) {
      ...
      addToast({
        ...
        description: message || "Error al eliminar el rol",
        color: "danger",
        timeout: 2000,
        variant: "bordered",
