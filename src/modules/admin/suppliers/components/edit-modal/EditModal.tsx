...
import { editSupplier, Supplier } from "@/modules/admin/suppliers";
...
export const EditModal = ({ textButton, size, supplier }: Props) => {
  ...
  const [full_name, setFull_name] = useState(supplier.full_name);
  const [ruc, setRuc] = useState(supplier.ruc);
  const [email, setEmail] = useState(supplier.email || "");
  const [address, setAddress] = useState(supplier.address || "");
  const [phone, setPhone] = useState(supplier.phone || "");
  const [image, setImage] = useState<File | null>(null);
  const [preview, setPreview] = useState<string | null>(supplier.image);
  const [state, setState] = useState<boolean>(
    supplier.state === 1 ? true : false,
  );
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    const { error, message, errors } = await editSupplier({
      id: supplier.id,
      data,
    ...
    if (error) {
      ...
      addToast({
        ...
        description: message || "Error al editar el proveedor",
        color: "danger",
        timeout: 2000,
        variant: "bordered",
  ...
  useEffect(() => {
    if (!isOpen) {
      setFull_name(supplier.full_name);
      setRuc(supplier.ruc);
      setEmail(supplier.email || "");
      setAddress(supplier.address || "");
      setPhone(supplier.phone || "");
      setState(supplier.state === 1 ? true : false);
      setImage(null);
      setPreview(supplier.image || null);
      setIsLoading(false);
      setErrors({});
      if (isOpen) closeAll();
    }
            ...
            {(onClose) => (
              <>
                <ModalHeader className="flex flex-col gap-1">
                  Editar Proveedor: {supplier.full_name}
                </ModalHeader>
                <ModalBody>
                  <Form
                    ref={formRef}
